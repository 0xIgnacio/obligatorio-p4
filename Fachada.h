...
class Fachada {
	...
	public:
		...
		void listarBruja(string, Error &);									// 4) Listar bruja en concreto, en caso de comun listar super tambien
		void listarSupremaMayor(Error &);								// 5) Listar suprema mas anciana
		void registrarHechizo(string, Hechizo *, Error &);	// 6) Agregar hechizo a bruja
		void listarHechizo(string, int, Error &);					// 7) Listar hechizo
		int hechizosEspecialesAno(string, int, Error &);		// 8) Cant hechizos especiales en determinado año
};
